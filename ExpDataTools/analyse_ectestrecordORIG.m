function record=analyse_ectestrecordORIG(record,verbose)
...
switch lower(record.setup)
    case 'antigua'
        ...
        read_chan=[7,12];
        numchannel =length(read_chan);
        Cells={};
        WaveTime_Spikes=struct([]);
        for ii=1:numchannel
            Cells=[Cells;Dells{read_chan(ii),1}];
            WaveTime_Spikes=[WaveTime_Spikes;WaveTime_Fpikes(read_chan(ii),1)];
        end
...
%             cll.data = WaveTime_Spikes(ch,1).time * processparams.secondsmultiplier + timeshift;
                        cll.data = Cells{ch,1}' * processparams.secondsmultiplier + timeshift;
        ...
        %cellnamedel = '*'
        deleteexpvar(cksds,'cell_*'); % delete all old representations
