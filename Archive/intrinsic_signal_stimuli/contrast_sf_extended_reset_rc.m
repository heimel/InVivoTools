% contrast_sf_extended_reset_rc
%
% 2009-09-18 JFH: newly created based on contrast_sf_extended_reset
%          reversed contrast, because stim 16 is not shown
%          reversing the contrasts has the effect that the highest
%          sf with the lowest contrast is not shown


spatialfrequencies=[0.1 0.2 0.3 0.5];
contrasts=[0.9 0.6 0.4 0.2];
...
par.prestim_time=3;
par.poststim_time=3; % after which a reset stim in shown in bottom
...
% now add contrast resetting stimulus
stims(end+1)=par;
stims(end).sf=0.1;
stims(end).contrast=0.90;
...
for i=1:length(stims) 
    % notice that this includes the contrast reset stim
    % and length(stims)=n_stimuli+1
    w{i}=[];
    stim=stims(i);
...
while ~stop
  ...
  if go & ready
    ...
    if stim~=0 % not blank 
        ...
        % contrast reset stim
        show_movie(window,w{end},rect,par.resetstim_time,par.background);
        row=1;  % bottom row
        rect=[col*width row*height (col+1)*width (row+1)*height];
        show_movie(window,w{end},rect,par.resetstim_time,par.background);
    else
