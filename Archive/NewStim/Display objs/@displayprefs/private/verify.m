...
if (mod(sz,2)==0),  % if a multiple of two or empty
	for i=1:2:sz,
		if (isfield(params,parameters{i})),  % if it is a field, add it if it makes sense
			if strcmp('rect', parameters{i}),
				if ~((isnumeric(parameters{i+1})) & prod((size(parameters{i+1}) == [ 1 4])*1)), %#ok<AND2>
					good = 0; errormsg = '''rect'' must be [x1 y1 x2 y2].'; 
				end;
			else
				if ~((isnumeric(parameters{i+1})) & prod((size(parameters{i+1}) == 1)*1)), %#ok<AND2>
	...
	good = 0; errormsg = 'parameters to displayprefs not of length modulo 2';
end;
