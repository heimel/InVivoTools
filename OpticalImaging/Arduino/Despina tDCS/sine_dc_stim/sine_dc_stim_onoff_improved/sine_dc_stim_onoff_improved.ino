...
void setup() {
  ...
  for (int i = 0; i < sample; i++) {
    f_sine[i] = ((f_sine[i] / 3.4) * voltage_out) + zerolevel - (4096 / 3.4) * (voltage_out / 2); //gives the right amplitude --> volt
  }
  ...
  switch (waveform) {
    ...
    case 2:  //DC Stimulation 
      ...
      if (trigger_tDCS) {
        ...
        int n_steps = 60;
        double stepsize = double(dcoutput/n_steps);
        if (trigger_tDCS) {
          trigger_tDCS = 0;
          for(int i=0; i<n_steps; i++){    // Slow Slope
             analogWrite(DAC1, zerolevel + int(i*stepsize));
             delay(25);
          }           
          analogWrite(DAC1, zerolevel + dcoutput);  //(correcting for the off-set)
