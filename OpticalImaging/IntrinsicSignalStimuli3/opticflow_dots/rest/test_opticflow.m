...
% 2007-12-14 JFH: on andrew: changed pre_stim back to 3s from 2s  
% unclear when it was originally changed, possibly doing the flavoprotein stuff (october 2006?)
% 2007-12-14 JFH: on andrew: changed stimtime back to 6s from 2s  

global whichScreen monitorframerate 

clear stims;
clear screen;
base_dir = 'C:\Documents and Settings\dataman\My Documents\MATLAB\scripts\';
thedir = char([base_dir,'MT_localizer']);
[numConds junk] = size(thedir);
pixRect = [0 0 768 768];  % size of the images to be presented
fmt = 'tif';
backGroundColor = uint8(255 - ((255-127)*(contrast/100)+127));  % change contrast
contrast = 100;
for cond = 1:numConds
...
width=round(r(3)-r(1));
height=round(r(4)-r(2));

dir_images = '\\Orange.nin.knaw.nl\mvp_data\Common\InVivo\Software\matlab\InVivoTools\trunk\intrinsic_signal_stimuli\MT_localizer';
