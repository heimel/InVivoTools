...
%        r_gamma_corr=255*( (0:1:255) /(255)).^0.44;
 %       g_gamma_corr=255*( (0:1:255) /(255)).^0.38;
  %      b_gamma_corr=255*( (0:1:255) /(255)).^0.44;
    ...
    case {'nori053'}, % stimulus pc for daneel imaging setup
        %still need to be linearized!
        warning('THIS MONITOR STILL NEEDS GAMMA CORRECTION. 2011-11-28');
        r_gamma_corr=255*( (0:1:255) /(255)).^0.357;
        g_gamma_corr=255*( (0:1:255) /(255)).^0.357;
        b_gamma_corr=255*( (0:1:255) /(255)).^0.357;
    otherwise
...
ptversion = Screen('version');
switch ptversion.major
    case 2
        Screen(window,'SetClut',clut);
    case 3
        Screen('LoadNormalizedGammaTable', window, clut/255);
end
