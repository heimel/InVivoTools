...
for i = 1:n_x*n_y
    ...
    pspar.rect = CenterRectOnPoint(default_rect,652,212);
    ...
    col=i-1-row*n_x;
    default_rect = [x_offset+col*width row*height x_offset+(col+1)*width (row+1)*height];
    pspar.rect = CenterRectOnPoint(default_rect,1360,337);
    % R = jander_screen_rotate(pspar.rect,10); %new line for rotation
    % pspar.rect=R; %new line for rotation
    pspar.nCycles = total_duration * pspar.tFrequency / length(angles);
    angles = angles( randperm(length(angles)) );
    ...
    col=i-1-row*n_x;
    default_rect = [x_offset+col*width row*height x_offset+(col+1)*width (row+1)*height];
    pspar.rect = CenterRectOnPoint(default_rect,560,742);
    % R = jander_screen_rotate(pspar.rect,10); %new line for rotation
    % pspar.rect=R; %new line for rotation
    pspar.nCycles = total_duration * pspar.tFrequency / length(angles);
    angles = angles( randperm(length(angles)) );
    ...
    col=i-1-row*n_x;
    default_rect = [x_offset+col*width row*height x_offset+(col+1)*width (row+1)*height];
    pspar.rect = CenterRectOnPoint(default_rect,1267,867);
    % R = jander_screen_rotate(pspar.rect,10); %new line for rotation
    % pspar.rect=R; %new line for rotation
    pspar.nCycles = total_duration * pspar.tFrequency / length(angles);
    angles = angles( randperm(length(angles)) );
