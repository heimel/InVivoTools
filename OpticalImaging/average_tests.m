...
if isempty(show)
  ...
  n_conditions=length(records(1).response);
  
  [m,norm_cond]=max(records(1).response);
  
  lconditions=[];
  lresponse=[];
  lresponse_sem=[];
  conditions_1=get_conditions_from_record(records(1));
  for i=1:length(records)
    if ~isempty(records(i).response)
      [conditions,condnames]=get_conditions_from_record(records(i));
      if normalized
	 ...
	 case 'tf',
	  ...
	  if isempty(normc)
	    normc=find(conditions==...
		       min(conditions(find(conditions>  0))));
	  end
  ...
  prevc=-1;
  for c=1:length(conditions)
    response(end+1)=mean( lresponse( find(lconditions==conditions(c))));
    response_sem(end+1)=sem( lresponse( find(lconditions==conditions(c))));
