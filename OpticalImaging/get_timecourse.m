...
%  
%  
%   2004, Alexander HeimelR
%
%   2005-01-02 JFH: Changed argument order and added ROR as argument
...
for cond=conditions
  % disp(['Reading condition: ' num2str(cond)]);
  condoffset=(cond-1)*fileinfo.n_images;
  start=1;
  for i=1:n_batches


    %fprintf(['Working on batch ' num2str(i) ' of ' num2str(n_batches)  ...
		%    ' : ']);
    ...
    if isempty(frames)
      disp(['Error: reading frames from ' fname ]);
      return
    end
    ...
    if ~isempty(pixelind_ror)
      ...
      disp(['IF A ROUTINE USES THIS, THE BEHAVIOR MAY BE ALTERED NOW' ...
	    ' 22 FEB 2006']);
     %old behaviour:  avg_ror=mean(frames(:,:),1)';
