...
if n_stims>15 
  disp('warning: stimulus setups can only start 15 different stimuli!');
end
...
%filename=complete_oi_path( [fname{1} 'B0.BLK']);
filename=[fname{1} 'B0.BLK'];

fileinfo=imagefile_info( filename );

if isempty(fileinfo.name)
	disp(['Warning: cannot open ' filename ]);
	fileinfo.n_conditions=length(conditions);
	fileinfo.cameraframes_per_frame=15;
	disp('Number of cameraframes per frames may be wrong');
	fileinfo.xbin=1;
	fileinfo.ybin=1;
	disp('Spatial binning may be wrong');
	%    fileinfo.name=filename;
end

if isempty(fileinfo.name)
	disp(['Error: cannot open ' filename ]);
	fileinfo.name=filename;
	%    return
else
...
if ~isfield(fileinfo,'frameduration')
	disp('Warning: defaulting frameduration to 0.6s');
	fileinfo.frameduration=0.6;
end


if length(conditions)~=fileinfo.n_conditions
	disp('PLOT_TESTRESULTS: number conditions in record does not match data');
...
if ~isempty(findstr(filename,'BLK'))
...
if length(ind)>1
	disp(['Error: two mice with name ' record.mouse ...
		' in mouse_db']);
	return
end
if isempty(ind)
...
if ~strcmp(birthdate,'unknown')
  try
    ...
    disp(['warning: could not get mouse age. wrongly entered in' ...
	  ' mouse_db?']);
    age='unknown';
  end
else
...
h_single_condition=show_single_condition_maps(record,fname,condnames,fileinfo,roi,ror,tit);
...
switch stim_type
 case 'sf_contrast'
   ...
   if ~isempty(findstr(record.comment,'contrast_sf'))
     contrast_sf=1;
   ...
   if ~contrast_sf
     ...
     for i=1:length(sf)
       ...
       c50=cfit(ind);
       disp(['c50 = ' num2str(c50*100) ' % ']);
       plot([c50 c50],[0 rfit{i}(ind)],[':' color(i)]);
     end


if 0
     % see if a shift works best
     [m,lowest_sf_ind]=min(sf)
     ind_c85=findclosest(cfit,0.85)
     for i=1:length(sf)
       shift(i)=findclosest(rfit{lowest_sf_ind},rfit{i}(ind_c85));
       xscale(i,:)=x(i,:)*shift(i)/85;
     end
	     shift
	     xscale
	curgcf=gcf
		figure;hold on
	     errorbar(x',y',dy','o');
	  	[rm,b,n]=naka_rushton(xscale(:)'/100,y(:)');
	       runifit = rm*cfit.^n./(b^n+cfit.^n);
		for i=1:length(sf)
		    min_err=inf;
		    for s=0.1:0.1:10
			err=0;
			scaled_cfit=s*cfit;
			for j=1:size(x,2)
			    ind=findclosest(scaled_cfit,x(i,j));
			    err=err+(runifit(ind)-y(i,j))^2;
                        end
                        if err<min_err
                           min_err=err;
			   best_scale=s;
		    ...
		    best_scale
	            ...
	            plot(cfit*100*best_scale,runifit,['-' color(i)]);
		end	
		set(gca,'XScale','log');
		xlim([14 100]);

	figure(curgcf)
	plot(cfit*100,runifit,['-b' ]);
end
 ...
 case {'sf','sf_temporal','sf_low_tf'}
		...
		acuity_sem=nan;
		if ~isempty(record)
			[acuity,acuity_sem]=get_valrecord(record,'sf_cutoff',mouse);
		end
		sf=(0.01:0.01:0.9);
	...
	case 'contrast',
		...
		%[contrast_threshold,rc,offset]=...
		%	cutoff_thresholdlinear([-0.1 -0.05 0 x(condind)],[0 0 0 y(condind)]);
		%contrast=(0:5:100);
		%plot(contrast,thresholdlinear( contrast/100*rc+offset),'k-');
		%text(15,ax(4)*0.9,[' threshold = ' ...
		%		num2str(contrast_threshold*100,2) ' %']);
	...
	case {'od','od_bin','od_mon'},
		...
		disp([ 'normalized C/I = ' num2str(cdi)]);
		disp([ 'ipsi= ' num2str( ip) ' contra= ' num2str(co) ' ratio= ' ...
			num2str(co/ip)]);
