function varargout=sigstar(groups,stats,nosort)
    ...
    %then we'll go easy on them and wrap it here rather than generate an error
...
function H=makeSignificanceBar(x,y,p)
    ...
    %     H(2)=text(mean(x(:)),starY,num2str(stars),...
    %         'HorizontalAlignment','Center',...
    %         'BackGroundColor','none',...
    %         'Tag','sigstar_stars', 'color', 'k', 'FontSize',fontsize);
    starY=mean(y)+myRange(ylim)*offset;
    H(2)=text(mean(x(:)),starY,stars,...
        'HorizontalAlignment','Center',...
        'BackGroundColor','none',...
        'Tag','sigstar_stars', 'color', [0.3 0.3 0.3], 'FontSize',fontsize, 'LineWidth',2);
    Y=ylim;
    if Y(2)<starY
...
function Y=findMinY(x)
    ...
    axis(gca,'tight')
    set(gca,'xlim',[x(1)-0.05, x(2)+0.05]) %Matlab automatically re-tightens y-axis
