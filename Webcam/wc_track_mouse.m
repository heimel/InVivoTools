function [freezeTimes, flightTimes] = wc_track_mouse(record,showShit,stimStart)
...
%
% 09-03-2015 Sven van der Burg, adapted by Alexander Heimel
...
blackThreshold = 0.30; % Treshold for amount of blackness to be treated as mouse
...
difTreshold = 0.2; % treshold + minimum movement for difference between frames to be considered as no difference, fraction of average movement
freezeTreshold = 1; % in seconds, treshold for
deriv2Tresh = 0.05; % Treshold for 2nd derivative of vidDif
noNest = 0;
...
for currentframenr = frameRange  %1:5:numFrames
    ...
    if isempty(pos)
        ...
        % Check whether the areasize of the found shape is larger then the
        % minimum
        maxAreaInd = find([pos.Area] == max([pos.Area]));
        maxAreaInd = maxAreaInd(1);
        nearMaxInd = ([pos.Area] > pos(maxAreaInd).Area * 0.5 & [pos.Area] > minAreaSize);
    ...
    if showShit
        ...
        pn=rectangle('Position', [0, 0, 1, 1], 'EdgeColor', 'g', 'FaceColor', 'g', 'Curvature', [1 1]);
...
fprintf('Flight detection is done \n');
