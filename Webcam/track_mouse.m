function [freezeTimes, nose, arse, stim, mouse_move, move_2der,trajectory_length,...
    averageMovement,minimalMovement,difTreshold,deriv2Tresh, freeze_duration, trajectory] =...
...
while vid.CurrentTime<=timeRange(2) && hasFrame(vid)
    ...
    vid.CurrentTime = vid.CurrentTime + 3; % jump 3s
    f = f+1;
end
...
vid.CurrentTime = timeRange(1);
currentframenr = 1; %vid.CurrentTime * frameRate;
...
while vid.CurrentTime < timeRange(2) && hasFrame(vid)
    ...
    if ~isempty(finalpos) && finalpos(1) ~= 0
        ...
        trajectory = [trajectory; [vid.CurrentTime finalpos([1 2])] ]; %#ok<AGROW> % t x y
...
%     smoothTrajectory(i,1) = mean(trajectory(i-trajectorySmoother(1):i+trajectorySmoother(2),2));
%     smoothTrajectory(i,2) = mean(trajectory(i-trajectorySmoother(1):i+trajectorySmoother(2),3));
% end

smoothTrajectory(:,1) = movmean(trajectory(:,2),2*trajectorySmoother(1));
smoothTrajectory(:,2) = movmean(trajectory(:,3),2*trajectorySmoother(1));
...
averageMovement = mean(smoothVidDif);
...
for i = 1:length(smoothVidDif)-1
    if smoothVidDif(i) < minimalMovement + difTreshold ...
            && abs(deriv2(i)) < deriv2Tresh
        if firstHit
            startTime = trajectory(i,1);
            firstHit = 0;
            hitnr = 1;
            vid.CurrentTime = trajectory(i,1);
        ...
        if hitnr/frameRate > freezeTreshold
            ...
            startf = @(t1,t2) rem([t1,t2],60);
            st = startf(startTime, stimStart);
            ...
            if isempty(snapframe)==0
                ...
                if  ~isempty(freezePeriodNr)
                    for k = freezePeriodNr
                        ...
                        p = 1;
                        framesforward = 0;
                        message1 = 'Click first on nose then on arse';
                        if k==1
                        ...
                        if startTime<ActStartTime || startTime>ActEndTime+0.2
                            uiwait(msgbox('press "space" for manual input, OK to continue'));
                                ...
                                while p<2   % looking at frames and checking
                                    ...
                                    switch button
                                        ...
                                        case 29 % right arrow
                                            ...
                                            image(snapframe,'Parent', snapaxes);
                                        case 110
                                            logmessage('No stimulus visible')
                                            stim(k,:) = [NaN NaN];
                                            p = p + 1;
                                        otherwise
                                            p = p + 1;
                                            if p==2
                                                uiwait(msgbox('Click on stim'));
                                                [xs(p), ys(p)] = ginput(1);
                                                stim(k,:) = [xs(p) ys(p)];
                                                hold on;
...
mouse_move = smoothVidDif;
move_2der = deriv2;
trajectory_length = length(mouse_move);
