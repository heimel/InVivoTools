...
nose = record.measures.nose;
arse = record.measures.arse;
stim = record.measures.stim;
...
for k = 1:size(nose,1);
    ...
    plot([0, arse_a(k,1)], [0, arse_a(k,2)], 'linewidth',3); hold on;
    grid on;
    extent1 = abs(arse_a)+100; ax1= max(max(extent1));
    plot([-ax1 ax1],[0 0],'--b',[0 0],[-ax1 ax1],'--b');
    ...
    if isempty(stim) == 0
        ...
        if stim_present(k)== 1
            ...
            plot([0, stim_a(k,1)], [0, stim_a(k,2)], 'linewidth',3); hold on;
            
            grid on;
            extent = abs(stim_a)+100; ax= max(max(extent));
            plot([-ax ax],[0 0],'--b',[0 0],[-ax ax],'--b');
            
            pos_theta(k) = get_ang(arse_a(k,:) , stim_a(k,:)) *sign(arse_a(k,2));
            text(50,100,sprintf('\\theta = %.1f,%c',pos_theta(k),char(176)),'color','b'); %char(176) is deg
