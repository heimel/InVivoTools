                ...
                if froze(r) == 1
                    if length(freeze_index)>1
                        for k = 1:length(freeze_index)
                            if (measures.freeze_duration(freeze_index(k)))'>3;
                                continue
                            else
...
figure((fig_n)+3); 
subplot(1,2,1); 
hand1 = rose(head_theta); title('head angle');
...
figure((fig_n)+4);
subplot(1,2,1); 
hand2 = rose(pos_theta); title('position angle');
set(hand2, 'linewidth', 2);
subplot(1,2,2);
hist(pos_theta, 20);

figure((fig_n)+5); title('freezing duration');
ax5 = subplot(1,2,1); 
...
if 0
    figure((fig_n)+6);plot(froze,'or');hold on;plot(freezing+0.1,'ob');plot(freezing_computed+0.2,'og');
