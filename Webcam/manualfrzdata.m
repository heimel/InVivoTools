        ...
        for m = 1:n_mice
            ...
            if ~isempty(db)
                ...
                for s = 1:2
                    switch s
                        case  discori
                            dbs1 = db(find_record(db,'stim_type=*disc_L*')); 
                            dbs2 = db(find_record(db,'stim_type=*disc_R*'));
                            dbs3 = db(find_record(db,'stim_type=*full*'));
                            dbs4 = db(find_record(db,'stim_type=*b_ellip_right*'));
                            dbs5 = db(find_record(db,'stim_type=*b_ellip_left*'));
                                                        
                            dbs = [dbs1 dbs2 dbs3 dbs4 dbs5];
                        ...
                        case discsmal || hawk
                            dbs1 = db(find_record(db, 'stim_type=*disc_corrS*'));
                            dbs2 = db(find_record(db, 'stim_type=*disc_s*'));
                            dbs3 = db(find_record(db, 'stim_type=*hawk_right*'));
                            dbs4 = db(find_record(db, 'stim_type=*hawk_left*'));
