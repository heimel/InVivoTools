...
dotSz = round(NewStimPixelsPerCm*NewStimViewingDistance*tan(MDp.dotsize*pi/180));
nFrames = round(fps_act*MDp.duration);

if strcmp(MDp.motiontype,'planar'),
    dpix_dt = 2*NewStimPixelsPerCm*NewStimViewingDistance*tan(MDp.velocity*pi/180)*1/fps_act;
    dxdy_dt=repmat(dpix_dt*[cos(pi*MDp.direction/180)/width sin(pi*MDp.direction/180)/height],...
        MDp.numdots,1);
else % must be radial
    dr_dt=2/sqrt(width^2+height^2)*NewStimPixelsPerCm*NewStimViewingDistance*tan(MDp.velocity*pi/180)*1/fps_act*...
