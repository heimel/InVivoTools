...
elseif ~ischar(figNum),  % is the call described above
    ...
    switch command,
        ...
        case 'UpdateRem',
            save_str=get_save_str; % a 'cd(pwd)' was originally pre-attached, not necessary?
            str = cat(1,update_str,save_str);
            b = sendcommand(scriptedstruct,str);
            if b,  % we know pathname is good
                pathn = rempath(scriptedstruct);
                fname = fullfile(pathn,'fromremote.mat');
                s=load(fname,'-mat');
        ...
        case 'Toremote',
            ...
            if b,
                ...
                if ~isempty(strs)
                    ...
                    pathn=rempath(scriptedstruct);
                    fname = fullfile(pathn,'toremote.mat');
                    evalin('base',['save(''' fname ''',' biglist ',''-v6'');']);
                    b=sendcommand(scriptedstruct,trans_str);
                    if b,
                        pathn = rempath(scriptedstruct);
                        fname = fullfile(pathn,'fromremote.mat');
                        s=load(fname,'-mat');
        ...
        case 'Toremoteload',
            ...
            trans_str = cat(1,trans_str,get_save_str);
            trans_str = cat(1,unloadstr,trans_str);
            b= checkremdir(scriptedstruct);
            if b,
                strs = lb_getselected(lbloc);
        ...
        case 'Tolocal',
            ...
            remsavestr = cat(1,{'g=5;'},get_save_str(ts) );
            b=sendcommand(scriptedstruct,remsavestr);
            if b,
                pathn = rempath(scriptedstruct);
                fname = [pathn 'fromremote.mat'];
        ...
        case 'Load',
            ...
            loadstr = cat(1,loadstr,get_save_str);
            b=sendcommand(scriptedstruct,loadstr);
            if b,
                pathn = rempath(scriptedstruct);
                fname = [pathn 'fromremote.mat'];
                s=load(fname,'-mat');
        ...
        case 'Duplicate',
            ...
            while (namenotfound),
                ...
                elseif ~isvarname(an), % syntax err
                    ...
                    dupstr=cat(1,dupstr,get_save_str);
                    b=sendcommand(scriptedstruct,dupstr);
                    if b,
                        pathn = rempath(scriptedstruct);
                        fname = [pathn 'fromremote.mat'];
                        s=load(fname,'-mat');
...
function save_str=get_save_str(vars)
if nargin<1
    vars='''s'',''NewStimPixelsPerCm'',''NewStimViewingDistance''';
end
save_str={  ...
...
if ~isfield(s,'s') % not all fromremote also include global variables, should be changed
    temps = s;
    s = [];
    s.s = temps;
end
strs=cell(length(s.s),1);
for i=1:length(s.s),
    ad = '';
    if s.s(i).loaded(i), ad = '*'; end;
    strs(i) = { [s.s(i).scripts ad] };
end;
set(lbrem,'Value',[]);
...
if ~isempty(z),
    ...
    % get NewStimGlobals from remotecomputer
    if isfield(s,'NewStimPixelsPerCm')
        set(ud.edtpixelspercm,'string',num2str(s.NewStimPixelsPerCm));
    end
    if isfield(s,'NewStimViewingDistance')
        set(ud.edtviewingdistance,'string',num2str(s.NewStimViewingDistance));
    end
    runexpercallbk('EnDis',z);
end;
disp('REMOTESCRIPTEDITOR: should parse NewStimGlobals here. and should be passed in fromremote');


function b = sendcommand(scriptedstruct,strs)
g = get(scriptedstruct.remdir,'String');
b = sendremotecommand(g,strs);
disp('REMOTESCRIPTEDITOR:')
disp(strs)

function pathn = rempath(scriptedstruct)
pathn=get(scriptedstruct.remdir,'String');
pathn = fixpath(pathn);

function b= checkremdir(scriptedstruct)
pathn=rempath(scriptedstruct);
