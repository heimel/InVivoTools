...
%  SCRIPTEDITOR
%
%  ScriptEditor brings up an editor for NewStim stimulus objects.  It
%  allows
...
persistent OpenFileDir;

if nargin==0, 	fig=figure;set(fig,'MenuBar','none','Name','Script Editor','numbertitle','off');
...
elseif ~ischar(figNum),  % is the call described above
    ...
    switch command,
        ...
        case 'NewOK',
            ...
            if isempty(str)
                str = [ty(1:10) '_' datestr(now,30)]; % truncating, because matlab 5 has variable length restrictions
            end
        ...
        case 'OpenFile',
            curdir=pwd;
            if isempty(OpenFileDir)
				OpenFileDir=fullfile(fileparts(which('NewStimInit')),'..','ScriptLibrary');
            end
            cd(OpenFileDir);

            [fname,pathn] = uigetfile({'*.mat','MAT-files (*.mat)';'*','All files (*)'},'Open file...');
            if fname(1)~=0,
                evalin('base',['load(fullfile(''' pathn ''',''' fname '''))'])
            end;
            ScriptEditor Update;
            ...
            if ischar(pathn) && exist(pathn,'dir')
                OpenFileDir=pathn;
            end
            cd(curdir);
        case 'Save',
			 curdir=pwd;
            if isempty(OpenFileDir)
				SaveFileDir=fullfile(fileparts(which('NewStimInit')),'..','ScriptLibrary');
			else
            ...
            if ~isempty(strs)
                ...
                if fname(1)~=0,
                    evalin('base',['save(''' pathn fname ''',''' trim(varnames) ''')']);
                end;
            end;
            cd(curdir);
...
uicontrol('Parent',h0, ...
    ...
    'FontWeight','bold', ...
    'ListboxTop',0, ...
    'Position',[16.3 213.2 201.7 25.9], ...
    'String','NewStim Script Editor:', ...
...
set(h0,'UserData',struct('lb',lb,'update',updatebt,'help',helpbt,'new',newbt,...
    ...
    'ScriptEditor','replace',replacebt));
