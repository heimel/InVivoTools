...
if nargin==0, 	
	fig=figure;set(fig,'MenuBar','none','Name','Stimulus Editor','numbertitle','off');
    drawStimEditor(fig);StimEditor('Update',fig);
elseif ~ischar(figNum),  % is the call described above
    ...
    switch command,
        ...
        case 'Load',
            ...
            %show stimulus
            testscript = StimScript(0);
            testscript=append(testscript,g);
            testscript=loadStimScript(testscript);
            MTI=DisplayTiming(testscript);
            DisplayStimScript(testscript,MTI,0);
            CloseStimScreen;
        ...
        case 'Replace',  % should really be renamed NewBasedOn
            % should only occur when 1 stimulus is selected
            namenotfound=1;
            prompt={'Name of new stimulus:'}; def = {''};
            dlgTitle = 'New stimulus name...';lineNo=1;
            while (namenotfound),
                answ=inputdlg(prompt,dlgTitle,lineNo,def);
                if isempty(answ), namenotfound = 0; %cancelled
...
uicontrol('Parent',h0, ...
    ...
    'String','NewStim Stimulus Editor:', ...
    'Style','text', ...
    'Tag','StaticText1');
...
%        'String','Close', ...
%        'Tag','Pushbutton1','Callback','StimEditor Close');
set(h0,'UserData',struct('lb',lb,'update',updatebt,'help',helpbt,'new',newbt,...
    ...
    'unload',unloadbt,'makescript',makescriptbt,'tag','StimEditor'));
