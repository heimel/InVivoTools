function S = mappingscript(PSparams,OLDSCRIPT)

% NewStim package: MAPPINGSCRIPT
%
%  SCRIPT = MAPPINGSCRIPT(PARAMETERS)
%
%  Creates a MAPPINGSCRIPT object, which is a descendant of the STIMSCRIPT
...
%
%  One may also use the construction SCRIPT=MAPPINGSCRIPT('graphical'), which
%  will prompt the user for all fields.  One may use the construction
%  SCRIPT=MAPPINGSCRIPT('graphical',OLDPERIODICSCRIPT), which will prompt the
%  user for all fields but provide the parameters of OLDMAPPINGSCRIPT as
%  defaults.  Finally, one may use SCRIPT=MAPPINGSCRIPT('default') to assign
...
if ischar(PSparams),
  ...
  [good,err] = verifymappingscript(PSparams);
                    ...
                    for n10=1:length(PSparams.barWidth),
                      for n11=1:length(PSparams.sPhaseShift),
                        for n12=1:length(PSparams.fixedDur),
						  ...
						  if n1==1
							  theParams.rect=[100 100 200 200];
						  end
...
if isempty(oldscript), 
  rect_str = '[0 0 640 480]';
  image_val = 3; 
  anim_val = 5; 
  flicker_val = 1; 
  chromhigh_str = '[255 255 255]'; chromlow_str = '[0 0 0]';
  angle_str = '[ 0:30:360-30 ]'; sFrequency_str = '[ 0.05 ]';
  tFrequency_str = '[ 2 ]';
  nCycles_str = '[ 5 ]';
  %           durtn_str = '[ 2 ]';
  % distance_str = '57';
  ...
  if get(cancel_ctl,'userdata')==1,
    ...
    if so
      ...
      psp = struct('imageType',imageType,'animType',animType,'flickerType',flickerType, ...
        ...
        'sPhaseShift',sPhase,'tFrequency',tFrequency, ...  %'distance',distance,
      ...
      [good, err] = verifymappingscript(psp);
