...
%DEZEEUW/TPREADCONFIG read twophoton experiment config file
%
% PARAMS. =
...
params.number_of_frames = length(dir(tpfilename(record,'*')));
params.NumberOfFrames = params.number_of_frames; % both are in use

params.NumberOfChannels = length(record.channels);
...
if isfield(inf,'ParsedImageDescription') && isfield(inf.ParsedImageDescription,'SecondsPerScanLine')
    ...
    warning('TPREADCONFIG:NO_SECONDSPERSCANLINE','No SecondsPerScanLine in tiff image description. Choosing arbitrary scanline time');
...
if ~isfield( inf, 'frame_timestamp')
    params.frame_timestamp = (0:(params.NumberOfFrames-1))*params.frame_period;
end
params.frame_timestamp__us = params.frame_timestamp * 1E6; % list of all frame timestamps in s

params.bidirectional = false; disp('TPREADCONFIG: hard coded bidirectional to false');
