...
% TPCORRECTTPTIMES - Corrects time for 2P frames
%   This is the Generic version, simply reports 2P frame times
%
...
%  associated with the folder as returned by 
...
try,
	triggerSignal = load([dirname filesep 'twophotontriggermethod.txt'],'-ascii');
...
if (onetriggerpercycle==0&total_param_frames~=length(spike2times))&~(triggerSignal==2),
	error(['Number of twophoton times (' int2str(length(spike2times)) ...
			...
			'); probably an extra trigger was recorded before acquisition; please remove any extra triggers from ' dirname filesep 'twophotontimes.txt.']);
end;
