...
h2=[loadIgor('r005_tet1_c01')';loadIgor('r005_tet1_c02')'; ...
    loadIgor('r005_tet1_c03')';loadIgor('r005_tet1_c04')';];
...
z = findcovdata(he,chi2inv(me.MEparams.threshcov,4),...
...
if me.MEparams.useabs,
   ...
   if me.MEparams.remove_unresolved,
      ppos_c = peakPos2(e',chi2inv(me.MEparams.thresh3,4),...
                max(1,floor(me.MEparams.peak_sep/dt)),0);
   ...
   ppos1= peakPos2(e_pos',chi2inv(thpos,4),...
                max(1,floor(me.MEparams.peak_sep/dt)),0);
		%floor(me.MEparams.overlap_sep/dt));
   ppos2= peakPos2(e_neg',chi2inv(thneg,4),...
                max(1,floor(me.MEparams.peak_sep/dt)),0);
		%floor(me.MEparams.overlap_sep/dt));
   ...
   if me.MEparams.remove_unresolved,
      ppos_cp = peakPos2(e_pos',chi2inv(me.MEparams.thresh3,4),...
                max(1,floor(me.MEparams.peak_sep/dt)),0);
      %find(ppos_cp>33402&ppos_cp<33429),
      ppos_cn = peakPos2(e_neg',chi2inv(me.MEparams.thresh3,4),...
                max(1,floor(me.MEparams.peak_sep/dt)),0);
		%floor(me.MEparams.overlap_sep/dt));
...
if me.MEparams.datadir~=0,
  majMin(majPosInd) = 2*me.MEparams.datadir-3;
  majMin(minPosInd) = 3-2*me.MEparams.datadir;
else, % all points are major
  majMin = 1:length(data);
end;
...
for k=1:nseg,
  [st,typ]=sd_procSpSeg(e,majMin,ppos,segst,k,dt);
...
stime2=[];  stype2=[];
if me.MEparams.datadir~=0,
  stime2=stime;stype2=stime;
  mjmn2 = find(mjmn==2*me.MEparams.datadir-3);
  stime = stime(mjmn2);
  stype = stype(mjmn2);
end;
