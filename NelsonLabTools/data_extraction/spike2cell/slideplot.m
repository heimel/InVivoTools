...
for r=1:rows
  for c=1:cols
    set(plothandles(r,c),'Position',[0.1+(c-1)*ud.w-0*ud.w*(ud.cols-ud.showc) ...
...
set(h0,'ResizeFcn',[ ...
   'if exist(''resizepos'') display(''ResizeFcn: Variable resizepos already exists.''); end;'...
   'if exist(''resizeud'') display(''ResizeFcn: Variable resizeud already exists.''); end;'...
   'resizepos=get(gcf,''Position''); resizeud=get(gcf,''UserData'');'...
   'if isfield(resizeud,''hhor''),'...
      'if exist(''resizehpos'') display(''ResizeFcn: Variable resizehpos already exists.''); end;'...
   ...
   'if isfield(resizeud,''hver''),'...
      'if exist(''resizevpos'') display(''ResizeFcn: Variable resizevpos already exists.''); end;'...
...
if(cols>ud.showc)
  set(hhor,'CallBack', [...
   'if exist(''moveud'') display(''CallBack: Variable moveud already exists.''); end;'...
   'if exist(''movepos'') display(''CallBack: Variable movepos already exists.''); end;'...
   'if exist(''moveh'') display(''CallBack: Variable moveh already exists.''); end;'...
   'if exist(''movec'') display(''CallBack: Variable movec already exists.''); end;'...
   'if exist(''mover'') display(''CallBack: Variable mover already exists.''); end;'...
   'moveud=get(gcf,''UserData''); ' ...
   'for mover=1:moveud.rows;for movec=1:moveud.cols;'...
   'moveh=moveud.plothandles(mover,movec);' ...
