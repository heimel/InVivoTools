function [newdd,status] = cleanupextract(dd,nameref,cksds,instruc);

%  [NEWDD,STATUS]=CLEANUPEXTRACT(DD,NAMEREF,THECKSDIRSTRUCT,INSTRUC)
%
...
for t=1:length(T),
  ...
  scratchfilename = [ p 'DD_' nameref.name '_' sprintf('%.4d',nameref.ref) ...
        '_' T{t} '_' dd.DDparams.scratchfile];
...
cellname = [dd.DDparams.event_type_string '_' nameref.name '_' sprintf('%.4d',nameref.ref) ...
             '_001_' expf dd.DDparams.scratchfile ];
cellname(find(cellname=='-'))='_'; disp(cellname)
%eval([cellname '= cksmultipleunit(intervals,'''','''',spiketimes,dd);']);
intervals,
thecell = cksmultipleunit(intervals,'','',spiketimes,dd);
