...
if isempty(db)
    disp('CONTROL_DB: No database selected or loaded');
    fig = [];
    return
end
...
% h.first = guicreate( button,'Tag','first','String','|<','visible','on',...
%     'callback','genercallback','top','top','width','auto',...
%     'height',buttonheight,'move','right','fontsize',8,...
...
%set(h.fig,'DeleteFcn',['control_db_callback(' num2str(h.dump) ')']);
left=left+buttonwidth+colsep;
maxleft=max(maxleft,left);
...
set(h.fig,'DeleteFcn',['control_db_callback(' num2str(h.close) ')']);
left=left+buttonwidth+colsep;
maxleft=max(maxleft,left);
% 
% 
...
%     'Callback','genercallback', ...
%     'ListboxTop',0, ...
%     'Position',[left top buttonwidth*1.2 buttonheight], ...
%     'String','Close figs', ...
%     'Tag','close figs',...
