...
%
%  2005-2024, Alexander Heimel
%

params = db_default_parameters();

if nargin<2
    color = params.db_backgroundcolor;
end
...
ud.top = top;
checkbox_width = buttonheight; %pxl
...
screensize = get(0,'ScreenSize');
if leftp > screensize(3)-params.db_figwidth
    leftp = 0;
    topp = topp-200;
end

h.fig = ...
    figure('Name','Database control',... %  'Color',params.db_backgroundcolor,...
    'WindowStyle','normal',...
...
left = left + buttonwidth/2 + params.db_colsep;
...
h.current_record = ...
    ...
    'ListboxTop',0, ...
    'Position',[left top width buttonheight], ... % [left top+4 48 buttonheight-8]
    'String',num2str(ud.current_record), ...
    'FontSize',ud.basefontsize,...
    'Tag','current',...
    'Tooltipstring','Current record number');
left = left + width + params.db_colsep;
maxleft = max(maxleft,left);

width = 8*ud.basefontsize;
h.count = uicontrol('Parent',h.fig, ...
    ...
    'ListboxTop',0, ...
    'Position',[left top width buttonheight], ... % [left top-4 width buttonheight-3]
...
% goto next line
top = top-buttonheight-params.db_colsep;
left = leftmargin;


critwidth= maxleft - 3*params.db_colsep - checkbox_width ;
h.crit = ...
    uicontrol('Parent',h.fig, ...
    'Style','edit',...
...
left=left+critwidth+params.db_colsep;
maxleft=max(maxleft,left);
...
h.filter = ...
    ...
    'Callback','genercallback', ...
    'ListboxTop',0, ...
    'Position',[left top checkbox_width  buttonheight], ...
    'FontSize',ud.basefontsize,...
    'String','', ...
    'Tag','filter',...
    'Tooltipstring','Toggle filter on and off');
left = left + checkbox_width + params.db_colsep;
maxleft = max(maxleft,left);
