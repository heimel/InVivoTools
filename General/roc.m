...
%
% Syntax: rocdata = roc(x,alpha,verbose)
%
...
%
% Minor changes by Alexander Heimel (2011)
%
...
nu=numel(args);
if isempty(nu)
  error('Warning: at least the data matrix is required')
elseif nu>3
...
% next two lines added by Alexander Heimel
yroc=[0; yroc];
xroc=[0; xroc];
...
d=realsqrt(xroc.^2+(1-yroc).^2); %apply the Pythagoras; theorem
...
if verbose
  ...
  elseif Area>=0.90 && Area<1
    str='Excellent test';
  elseif Area>=0.80 && Area<0.9
    str='Good test';
  elseif Area>=0.70 && Area<0.8
    str='Fair test';
  elseif Area>=0.60 && Area<0.7
    str='Poor test';
  elseif Area>=0.50 && Area<0.6
    str='Fail test';
  else
