...
%  curve
%
%  2003-2020, Alexander Heimel 
%

if nargin<5 || isempty(spont_hint)
    spont_hint = min(curve(2,:));
end
if nargin<4 || isempty(peak_hint)
    peak_hint = max(curve(2,:));
end
if nargin<3 || isempty(tw_hint)
    tw_hint = 30/360;
end
if nargin<2 || isempty(pref_hint)
    [~,i] = max(curve(2,:));
    pref_hint = curve(1,i);
end
...
if norm_error<0 || ~isreal(norm_error)
    logmsg(['Normalized fitting error: ' num2str(norm_error,3)]);
end
