...
if nargin<5 req=[0 0];end;
if length(req)<2 req=[0 0]; end;
if (1-iscell(scls))&(1-isreal(scls))  GS=gaborspaceF(S,scls); return; end;
% building the filter
if (1-iscell(scls))&(isreal(scls))
    if length(scls)==3
...
for k=1:ns
   ...
   T=round(beta*sigma);
   D=-T:T;
   filter{k}=(norm/sigma)*exp(-(D.^2/sigma^2)-i*(2*pi*freqs(k))*D);
   switch req(1)
