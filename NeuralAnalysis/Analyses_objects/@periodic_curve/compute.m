...
for z=1:length(vars),  % looping over second parameter
    ...
    for j=1:length(vals{z}), % looping over first parameter
        ...
        loc1 = findclosest(c.fftfreq{1},1/estper); % since its multiple pres. of same stim, take first
        loc2 = findclosest(c.fftfreq{1},2/estper); % since its multiple pres. of same stim, take first
        cf1mean=[]; cf1stddev=[]; cf1stderr=[];
        cf2mean=[]; cf2stddev=[]; cf2stderr=[];
        cf1f0mean=[]; cf1f0stddev=[]; cf1f0stderr=[];
        cf2f1mean=[]; cf2f1stddev=[]; 
        for nn=1:nCyc,
    ...
    cf1f0meanc=[]; cf1f0stddevc=[]; cf1f0stderrc=[]; cf2f1meanc=[]; cf2f1stddevc=[]; 
    ...
    for nn=1:length(stims),
        loc1c = findclosest(c1.fftfreq{nn},1/estpers(nn));
        loc2c =findclosest(c1.fftfreq{nn},2/estpers(nn));
        ...
        loc1 = findclosest(c.fftfreq{nn},1/estpers(nn));
        loc2 = findclosest(c.fftfreq{nn},2/estpers(nn));
