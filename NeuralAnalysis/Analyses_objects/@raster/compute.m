...
%   if cstop{k}>length(bins{k})
%       disp(['RASTER\COMPUTE: cstop ' num2str(cstop{k}-length(bins{k})) ' samples too long.']);
%   end
...
for k=1:K,
    ...
    for i=1:length(trigs{k}),
      ...
      if ~isempty(n),
		  ...
		  rast_x = cat(1,rast_x,g-trigs{k}(i)); %changed from 2 to 1 by Alexander Heimel
		  rast_y = cat(2,rast_y,repmat(i,1,length(g)));
          %cind, size(n),
	  zzzzz=sum(n(cind{k}));
          nncounts{k}(i) = zzzzz;
	  else
          vals{k}(:,i) = 0; fftvals{k}(:,i) = 0; nncounts{k}(i) = 0;
      end;
    end;
    ...
    rast{k} = [ rast_x'; rast_y]; %added ' by Alexander Heimel
