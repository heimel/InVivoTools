...
switch lower(record.setup)
    case 'antigua'
        ...
        %         channels_to_read = [6:8];
        %         Dells=EVENT.snips.Snip.times;
        %         for j=1:length(channels_to_read)
        %             Cells{j,1}=Dells{channels_to_read(j),1};
        %         end
        Cells=EVENT.snips.Snip.times;
        EVENT.Myevent = 'Snip';
        EVENT.type = 'snips';
        EVENT.Triallngth = EVENT.timerange(2)-EVENT.strons.tril(1)+(1/EVENT.snips.Snip.sampf);
        EVENT.Start = 0;
        WaveTime_Spikes = ExsnipTDT(EVENT,EVENT.strons.tril(1));
        
        if  exist(fullfile(datapath,EVENT.Myblock,'cluster_spikes.mat'))
            load(fullfile(datapath,EVENT.Myblock,'cluster_spikes.mat'));
        elseif strmatch(record.stim_type,'sg')==1 && ~ exist(fullfile(datapath,EVENT.Myblock,'cluster_spikes.mat'))
            cluster_spikes=struct([]);
            for j=1:length(WaveTime_Spikes)
                clear('cll');
                cll.sample_interval = 1/EVENT.snips.Snip.sampf;
                sp_su=WaveTime_Spikes(j,1).data;
                cll = get_spike_features(sp_su, cll );
                X=[cll.spike_amplitude,cll.spike_trough2peak_time,cll.spike_peak_trough_ratio,cll.spike_prepeak_trough_ratio,cll.spike_lateslope];
            [cluster_spikes(1,j).NumClusts,cluster_spikes(1,j).IDX,cluster_spikes(1,j).Cent] = optnum_clust(X);
            end
            save(fullfile(datapath,EVENT.Myblock,'cluster_spikes.mat'),'cluster_spikes')
        else
            sprintf('WARNING: first you must run the receptive field analysis')
            return
        end
        clear X;
        clear('cll');
        ...
        for ch = 1:length(Cells)
            ...
            cll.index = ch; % will be used to identify cell
            cll.data = Cells{ch}' * processparams.secondsmultiplier + timeshift;
            ...
            if cluster_spikes(1,ch).NumClusts==1
                cll.name_su{1}=sprintf('cell_%s_%s_%.4d_%.3d_%.3d',...
                    acqinfo(1).name,unitchannelname,acqinfo(1).ref,cl,1);
                cll.wave_su{1} = cll.wave;
                cll.std_su{1} = cll.std;
                cll.snr_su{1} = cll.snr;
                cll.spike_amplitude_su{1}=cll.spike_amplitude;
                cll.spike_trough2peak_time_su{1}=cll.spike_trough2peak_time;
                cll.spike_peak_trough_ratio_su{1}=cll.spike_peak_trough_ratio;
                cll.spike_prepeak_trough_ratio_su{1}=cll.spike_prepeak_trough_ratio;
                cll.spike_lateslope_su{1}=cll.spike_lateslope;
            else

%                 [COEFF, SCORE] =
%                 princomp(X);fffsc=SCORE*COEFF;[cll.NumClusts,cll.IDX] = optnum_clust(fffsc);

                for k=1:cluster_spikes{1,ch}.NumClusts
                    cll.name_su{k}=sprintf('cell_%s_%s_%.4d_%.3d_%.3d',...
                acqinfo(1).name,unitchannelname,acqinfo(1).ref,cl,k);
                    Spikes = spikes(cluster_spikes{1,ch}.IDX==k,:);
                    cll.wave_su{k} = mean(Spikes,1);
                    cll.std_su{k} = std(Spikes,1);
                    cll.snr_su{k} = (max(cll.wave_su{k})-min(cll.wave_su{k}))/mean(cll.std_su{k});
                end;
            end
            %   cells(1,cl) = cll;
    ...
    if cluster_spikes
        ...
        for i=2:n_cells
            ...
            for j=1:i-1
                ...
                [dummy,Ulabel] =fcm(features,2,[2 50 1e-4 0]);
