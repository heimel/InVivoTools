function CSnip = Exsnip(EVENT, Trials)
...
if ~isfield(EVENT.snips, EVENT.Myevent)
...
Sampf = EVENT.snips.(EvCode).sampf; %sample frequency for this event
Evlngth = EVENT.snips.(EvCode).size;    %number of samples in each event epoch
Evtime = Evlngth/Sampf; %timespan of one event epoch
if iscell(EVENT.snips.(EvCode).channels)
    ChaNm = length(EVENT.snips.(EvCode).channels); %channels in block is a cell
else
    ChaNm = EVENT.snips.(EvCode).channels; 
end
   ...
   for j = 1:size(Trials,1)
        ...
        %Times = Times - Trials(j,1);
             ...
             for i = Chans
                 ...
                 CSnip(Nc,j) = { struct('time', num2cell(T2), 'data',  num2cell(D2, 1)  ) };
