function SIG = Exd3(EVENT, Trials)
...
if ~isfield(EVENT.strms, EVENT.Myevent)
...
%EvCode = EVENT.strms{IEv}; %event code as string
EvCode = EVENT.Myevent;
Sampf = EVENT.strms.(EvCode).sampf; %sample frequency for this event
Evlngth = EVENT.strms.(EvCode).size; %number of samples in each epoch
Evtime = Evlngth/Sampf; %timespan of one event epoch plus one for safety
ChaNm = EVENT.strms.(EvCode).channels; %channels in block
