function [gratingid, gratingrect] = CreateProceduralGrating(windowPtr, width, height, backgroundColorOffset, radius, contrastPreMultiplicator, imageType, phaseOffset)
% [gratingid, gratingrect] = CreateProceduralGrating(windowPtr, width, height [, backgroundColorOffset =(0,0,0,0)] [, radius=inf][, contrastPreMultiplicator=1][, imageType=2][, phaseOffset=0])
%
...
% History:
% Copied from CreateProceduralSineGrating from PTB3 (MK)
...
if nargin < 7 || isempty(imageType)
    imageType = 2; % i.e. sinusoid
end

if nargin < 8 || isempty(phaseOffset)
    phaseOffset = 0; 
end
...
glUniform1f(glGetUniformLocation(gratingShader, 'PhaseOffset'), phaseOffset);
glUniform1i(glGetUniformLocation(gratingShader, 'ImageType'), imageType);
