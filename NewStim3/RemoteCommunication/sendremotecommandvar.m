...
if checkremotedir(pathstring), % directory exists
	...
	fout  = [pathn 'fromremote'];
	fin  =  [pathn 'toremote'];
	if exist(fname)==2, delete(fname); end;
	if exist(fout)==2, delete(fout); end;
	if exist(fnameerror)==2, delete(fnameerror); end;
	...
	if b,
		...
		if b & exist(fout)==2, vars = load(fout,'-mat'); else, vars = []; end;
	end;
end;

if b==0, vars = []; end;

if b|errorflag, try, delete([fixpath(Remote_Comm_dir) filesep 'runit.m']); end; end;
...
if v>13, savestr = ' -V6 -mat '; else, savestr = ' -mat '; end;
savestr,
