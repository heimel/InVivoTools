...
if checkremotedir(Remote_Comm_dir), % directory exists
	if exist([fixpath(Remote_Comm_dir) 'gotit'])==2,
		delete([fixpath(Remote_Comm_dir) 'gotit']); end;
	if exist([fixpath(Remote_Comm_dir) 'fromremote'])==2,
        	delete([fixpath(Remote_Comm_dir) 'fromremote']); end;
	if exist([fixpath(Remote_Comm_dir) 'scripterror'])==2,
		delete([fixpath(Remote_Comm_dir) 'scripterror']); end;
	b = writeremote(strs);
	if b,
