...
g = struct(dp);
if std(cat(1,g.fps))~=0
    fpsvstr = 'multiply valued'; 
    fpsstr = '0';
else
    fpsvstr = num2str(g(1).fps); 
    fpsstr = fpsvstr; 
end
if std(cat(1,g.BGpretime))~=0
    bgprevstr='multiply valued'; 
    bgprestr='0';
else
    bgprevstr=num2str(g(1).BGpretime); 
    bgprestr = bgprevstr; 
end
if std(cat(1,g.BGposttime))~=0
    bgpostvstr='multiply valued'; 
    bgpoststr ='0';
else
    bgpostvstr=num2str(g(1).BGposttime);
    bgpoststr=bgpostvstr; 
end
sz = size(cat(1,g.rect));
if any(std(cat(1,g.rect))~=0) &&(sz(1)~=1)
    rectvstr = 'multiply valued';
    rectstr = '[0 0 0 0]';
else
    rectvstr = mat2str(g(1).rect);
    rectstr = rectvstr;
end
if std(cat(1,g.roundFrames))~=0
    rframevstr='multiply valued'; 
    rframeval=1;
else
    rframevstr = num2str(g(1).roundFrames);
    rframeval = 1+g(1).roundFrames;
end
if std(cat(1,g.forceMovie))~=0
    forceMovvstr = 'multiply valued';
    forceMovval=1;
else
    forceMovvstr = num2str(g(1).forceMovie);
...
while ~error_free
    ...
    if get(cancelctl,'userdata')==1
        ...
        if get(rframeDctl,'value')==1
            ...
            dpStr = [dpStr ',''roundFrames'', ' int2str(roundFrames)]; %#ok<AGROW>
        end
        if get(forceMovDctl,'value')==1
            forceMovie = get(forceMovctl,'value')-1;
            dpStr = [dpStr ',''forceMovie'', ' int2str(forceMovie)]; %#ok<AGROW>
        end
        if get(bgpreDctl,'value')==1
            try
                str = get(bgprectl,'String');
                bgpretime = eval(str);
                dpStr = [dpStr ',''BGpretime'', ' str ]; %#ok<AGROW>
            catch
                errordlg('Syntax error in BGpretime');
                so=0; 
            end
        end
        if get(bgpostDctl,'value')==1
            try
                str = get(bgpostctl,'String');
                bgposttime = eval(str);
                dpStr = [dpStr ',''BGposttime'', ' str ]; %#ok<AGROW>
            catch
                errordlg('Syntax error in BGposttime');
                so=0; 
            end
        end
        if ~isempty(dpStr)
        ...
        if so % if no syntax errors, check parameter values
            noEr = 1;
            try
                eval(['testDP=displayprefs({' dpStr '});']);
            catch me
                errordlg(['Error: check parameters: ' me.message]);
                noEr = 0;
            end
            if noEr
                error_free = 1;
