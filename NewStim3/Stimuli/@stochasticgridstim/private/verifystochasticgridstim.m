...
if proceed
    ...
    if f
        proceed = 0;
        errormsg = 'R,G,B in BG must be in [0..255]';
    end
    f = find(parameters.values<0|parameters.values>255);
    if f
        proceed = 0;
        errormsg = 'R,G,B in values must be in [0..255]';
    end
end
...
if proceed % check that value and dist have same size
    ...
    if ~proceed
        errormsg = 'dist and value must have same number of rows.';
    else
        f = find(parameters.dist<0);
        proceed = isempty(f);
        if ~proceed
            errormsg = 'all entries of dist must be non-negative.';
        end
    end
end

if isfield(parameters,'angle')
    if ~isnumeric(parameters.angle)
        proceed = 0;
        errormsg = 'angle must be a number.';
    end
end
...
if proceed % check displayprefs for validity
    try
        dummy = displayprefs(parameters.dispprefs);
    catch
        proceed = 0;
        errormsg = ['dispprefs invalid: ' lasterr];
    end
end
