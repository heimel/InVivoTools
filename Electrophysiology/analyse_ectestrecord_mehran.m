...
%         EVENT.strons.tril(1) = EVENT.strons.tril(2);
%         EVENT.strons.tril(1) = EVENT.strons.OpOn(1);
...
%         EVENT.strons.tril(1) = EVENT.strons.tril(5);
        ...
        EVENT.type = 'snips';
        EVENT.Start = 0;
%         EVENT.snips.Snip.channels = 1:16;
...
%                     WaveTime_Fpikes(i,1).data = [WaveTime_Fpikes(i,1).data; WaveTime_chspikes(channels2analyze(i),1).data];
        ...
        % temporary for optostim
        if ~isempty(stimsfile.MTI2{end}.frameTimes)
            intervals=[stimsfile.start ...
                stimsfile.MTI2{end}.frameTimes(end)+10];
        else
            intervals=[stimsfile.start stimsfile.start+600];
        end
