...
if isempty(rc)
    msg = ['Could not compute reverse correlation for ' recordfilter(record)];
...
% store normalized receptive field plot
which_int=(1: floor(para_rc.interval(2)/para_rc.timeres));
rf={};
for i=which_int
  rf{i}=squeeze(rcs.rc_avg(1,i,:,:,end));
end
measures.rf=rf;
rf=rf{1};
...
disp(['ANALYSE_SG: Center pixel = ' mat2str( measures.rf_center_pixel ) ]);
...
if rfx<=1 || rfx>=x || rfy<=1 || rfy>=y
  disp('ANALYSE_SG: RF center is on edge of screen.');
  measures.usable=0;
end
...
xlabel(['# spikes ' num2str(hist_start) '-' num2str(hist_end) 's after frame' ]);
  set(gca,'YScale','log');
  disp(['Number of no onframes: ' num2str(length(no_onframetimes))]);
  
  disp(['Number of onframes   : ' num2str(length(onframetimes))]);
...
if isempty(i2)
    disp('ANALYZE_SG: No response at all.');
...
if length(inp.st.mti)==1
  ...
  elseif recording_interval(2)-stimulus_interval(2)>3
    ...
    disp('Recorded too little time before or after stimulation to compute spontaneous rate');
    measures.rate_spont=nan;
  end
else
  disp('spontaneous rate calculation not suitable for multiple repetitions');
  measures.rate_spont=nan;
end
...
if length(monitorpos)==2 % only x and y
    ...
    disp('ANALYSE_SG: Getting viewing distance from record monitor position.');
end
