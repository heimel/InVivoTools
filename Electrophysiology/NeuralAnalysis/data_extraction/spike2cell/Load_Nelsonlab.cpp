...
* version 5.1
*
* Reads .spikes and .spiketimes files generated by spike2cell (JAF Heimel) from NelsonLab data
*
* based on and parts from LoadTT_NeuralynxNT by ADR May 2002
%
*--------------------------------*/
...
int GetNumberOfSpikes(char* fn){
  ...
  fread(&n_spikes,1,sizeof(int),fp);

  return 0;
  fread(&n_channels,1,sizeof(int),fp);


  fread(&before,1,sizeof(int),fp);
  fread(&after,1,sizeof(int),fp);
  spikewindow=before+after;

  
  mexPrintf("Reading file %s:\nspikewindow = %d,  %d spikes, %d channels.\n",
	    fn,spikewindow , n_spikes, n_channels);
