function measures = compute_adaptation_measures( record )
%COMPUTE_ADAPTATION_MEASURES compute some specific adaptation measures
%
%  MEASURES = COMPUTE_ADAPTATION_MEASURES( RECORD )
%
%
...
for i=1:length(measures)
    m = measures(i);
    [~,ind_max] = max(m.psth_count{1});
    peak_time = m.psth_tbins{1}(ind_max);
   ...
   for j = 1:length(binstarts)
       ind = (m.psth_tbins{1}>binstarts(j) & m.psth_tbins{1}<binends(j));
       peakrates(j) = mean(m.psth_count{1}(ind));
   end % j
...
%    hold off
   ...
   measures(i).adaptation_peak2 = peakrates_normalized(2);
   measures(i).adaptation_peak3 = peakrates_normalized(3);
   measures(i).adaptation_peak4 = peakrates_normalized(4);
   measures(i).adaptation_peak5 = peakrates_normalized(5);
