...
%LOAD_INTAN loads intan data into invivotools
%
% EVENT = LOAD_INTAN(EVENT, MATFILENAME)
%
% Based on read_Intan_RHD2000_file
%
% 201X, Mehran Ahmadlou
...
EVENT.Strms = [];
EVENT.Snips = [];
EVENT.Snips.rawsig = amplifier_data;
% EVENT.Snips.rawsig = amplifier_data - repmat(amplifier_data(21,:),[32,1]); % temporarily
EVENT.Snips.rawtime = t_amplifier;
EVENT.snips.Snip.channels = 1:size(EVENT.Snips.rawsig,1);
% Visual stim trigs
C = (board_adc_data(1,:)<0.4);
CC = [diff(C),0];
CCC = (CC==1);
Ttime = EVENT.Snips.rawtime(CCC);
EVENT.strons.tril = Ttime;

% Opto ON trigs
C = (board_adc_data(2,:)>1);
CC = [diff(C),0];
CCC = (CC==1);
Ctime = EVENT.Snips.rawtime(CCC);
if ~isempty(Ctime) || length(Ctime)>2
EVENT.strons.OpOn = Ctime;
end
