...
if StartAtStep <=2
    ...
    for iSub = 1:nFiles
        ...
        if nSpikes > ToSpikes
            Overlap = 0;
            nFiles = find(repmat(nSpikes,1,20)./(1:20) < ToSpikes, 1 );
            ...
            for iSplit = 1:length(FDFilesToSplit)
				...
				for ii=1:nFiles
                    ...
                    % this doesn't work - ADR 11/11/201
					%for ib = 1:nFiles 
						bstart = min(1+(ii-1)*ToSpikes,nSpikes);
...
if StartAtStep <= 3 & strcmp(Do_AutoClust,'yes')
    ...
    for iCrFDs = 1:nFiles   
		...
		if CalcFD
            ...
            % Removed extra normalization ADR 2010 
% 			for iFD = 1:nF
...
if strcmp(Do_AutoClust,'yes')
   ...
   for i = 1:nFiles
       ...
       elseif(strcmpi(gPar.ClusterAlgorithm,'KlustaKwik')) 
           ...
           % adr should use delete function
           % modified adr 2010-11-11 to not remove these files
           if exist(fullfile(INpath, [INname, '.fet.1']), 'file')
               delete(fullfile(INpath, [INname, '.fet.1']));
           end
           %if exist(fullfile(INpath, [INname, '.fd']), 'file')
           %    delete(fullfile(INpath, [INname, '.fd']));
           %end
