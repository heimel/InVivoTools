...
function [redraw, rekey, undoable] =ShowAverageWaveform(iClust,varargin)
...
if run_avg_waveform
    ...
    for it = 1:4
        ...
        hold on;
        plot(xrange,mWV(it,:)+sWV(it,:),'w','LineWidth',1);
        plot(xrange,mWV(it,:)-sWV(it,:),'w','LineWidth',1);
        plot(xrange, mWV(it,:));
        %errorbar(xrange,mWV(it,:),sWV(it,:)) %,'blue');
        plot(xrange,mWV(it,:),'LineWidth',2,'Color',MClust_Colors(iClust + 1,:));%,sWV(it,:)) %,'blue');
